...
ocl_args::ocl_args(cl_device_type deviceType)
	: context(NULL)
	, device(NULL)
	, commandQueue(NULL)
	, program(NULL)
...
* Dependencies: None
*/
int ocl_args::SetupOpenCL(cl_device_type deviceType)
{
...
int ocl_args::CreateAndBuildProgram(const std::string& filename)
{
	cl_int err = CL_SUCCESS;
...
* in the same program.
*/
int ocl_args::GetPlatformAndDeviceVersion(cl_platform_id platformId)
{
	cl_int err = CL_SUCCESS;
...
cl_uint ocl_args::ExecuteKernel(size_t *globalWorkSize, cl_uint workSizeCount, size_t* localWorkSize)
{
	cl_int err = CL_SUCCESS;
...
cl_uint ocl_args::UpdateProfiler()
{
